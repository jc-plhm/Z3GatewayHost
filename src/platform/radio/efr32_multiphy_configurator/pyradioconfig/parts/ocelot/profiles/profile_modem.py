...
def buildModemAdvancedInputs(model, profile, family):
    ...
    IProfile.make_hidden_input(profile, model.vars.freq_offset_hz                    , 'Advanced', readable_name="Frequency Offset Compensation (AFC) Limit",                          value_limit_min=0,      value_limit_max=500000, units_multiplier=UnitsMultiplier.KILO)
    ...
    IProfile.make_deprecated_input(profile, model.vars.pll_bandwidth_tx                  , 'Advanced', readable_name="PLL Bandwidth in TX mode")
    IProfile.make_deprecated_input(profile, model.vars.pll_bandwidth_rx                  , 'Advanced', readable_name="PLL Bandwidth in RX mode")
    ...
    IProfile.make_hidden_input(profile, model.vars.number_of_timing_windows, 'Advanced',
                            readable_name="Number of Timing Windows to Detect", value_limit_min=1, value_limit_max=16)
    ...
    IProfile.make_hidden_input(profile, model.vars.frequency_offset_period           , 'Advanced', readable_name="Frequency Offset Period",                      value_limit_min=0,      value_limit_max=7)
    IProfile.make_hidden_input(profile, model.vars.afc_period                        , 'Advanced', readable_name="Frequency Offset Compensation (AFC) Period",                            value_limit_min=0,      value_limit_max=7)
    ...
    IProfile.make_hidden_input(profile, model.vars.agc_settling_delay, 'Advanced', readable_name="AGC Settling Delay",
                               value_limit_min=0, value_limit_max=63)
    IProfile.make_hidden_input(profile, model.vars.afc_step_scale                    , 'Advanced', readable_name="Frequency Offset Compensation (AFC) Step Scale",                               value_limit_min=0.0,    value_limit_max=2.0, fractional_digits=2)
    ...
    IProfile.make_deprecated_input(profile, model.vars.rx_bitrate_offset_hz              , 'Advanced', readable_name="RX Baudrate offset",                           value_limit_min=0,      value_limit_max=200000)
    IProfile.make_hidden_input(profile, model.vars.agc_speed, 'Advanced', readable_name="AGC Speed")
    IProfile.make_hidden_input(profile, model.vars.frequency_comp_mode               , 'Advanced', readable_name="Frequency Compensation Mode")
    if family != "dumbo":
        IProfile.make_deprecated_input(profile, model.vars.src_disable                       , 'Advanced', readable_name="SRC Operation")
        ...
        if family != "ocelot":
            IProfile.make_hidden_input(profile, model.vars.target_osr, 'Advanced',readable_name="Target oversampling rate", value_limit_min=3, value_limit_max=8)
        else:
            #BCR demod in Ocelot allows for much larger OSR
            IProfile.make_hidden_input(profile, model.vars.target_osr, 'Advanced',readable_name="Target oversampling rate", value_limit_min=3, value_limit_max=127)

    IProfile.make_hidden_input(profile, model.vars.ook_slicer_level                  , 'Advanced', readable_name="OOK slicer level",                               value_limit_min=1,     value_limit_max=10)
    ...
    IProfile.make_hidden_input(profile, model.vars.ircal_rxtx_path_common, 'Advanced', readable_name="Common RX/TX circuit")
    IProfile.make_hidden_input(profile, model.vars.ircal_power_level, 'Advanced', readable_name="IR cal power level (amplitude)",                               value_limit_min=0,     value_limit_max=255)
...
def buildModemInfoOutputs(model, profile):
    ...
    profile.outputs.append(ModelOutput(model.vars.div_antdivmode, '', ModelOutputType.INFO, readable_name='Antenna diversity mode'))
    profile.outputs.append(ModelOutput(model.vars.div_antdivrepeatdis, '', ModelOutputType.INFO, readable_name='Disable repeated measurement of first antenna when Select-Best algorithm is used'))
...
def buildRailOutputs(model, profile):
    ...
    profile.outputs.append(ModelOutput(model.vars.psm_max_sleep_us, '', ModelOutputType.RAIL_CONFIG, readable_name='Maximum time to sleep in PSM (us)'))
    profile.outputs.append(ModelOutput(model.vars.am_low_ramplev, '', ModelOutputType.RAIL_CONFIG, readable_name='LOW ramp level for amplitude modulation'))
