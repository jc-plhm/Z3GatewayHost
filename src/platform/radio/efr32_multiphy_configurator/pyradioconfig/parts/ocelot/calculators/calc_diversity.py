...
class CALC_Diversity_Ocelot(CALC_Diversity):
    ...
    def calc_antdivrepeatdis_reg(self, model):
        antdivmode = model.vars.antdivmode.value
        antdivrepeatdis = model.vars.antdivrepeatdis.value

        reg = 1 # : Disable repeat measurement of 1st antenna by default

        # : For select-best algorithms, set repeat first register
        if antdivmode == model.vars.antdivmode.var_enum.ANTSELCORR or \
                antdivmode == model.vars.antdivmode.var_enum.ANTSELRSSI:
            if antdivrepeatdis == model.vars.antdivrepeatdis.var_enum.REPEATFIRST:
                reg = 0

        if antdivmode == model.vars.antdivmode.var_enum.DISABLE or \
                antdivmode == model.vars.antdivmode.var_enum.ANTENNA1 or \
                antdivmode == model.vars.antdivmode.var_enum.ANTSELFIRST:
            self._reg_do_not_care(model.vars.MODEM_CTRL3_ANTDIVREPEATDIS)
        else:
            self._reg_write(model.vars.MODEM_CTRL3_ANTDIVREPEATDIS, reg)
