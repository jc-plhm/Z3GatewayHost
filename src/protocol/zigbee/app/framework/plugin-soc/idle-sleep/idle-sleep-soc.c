...
#define CURRENT_MODULE_NAME    "ZIGBEE_PLUGIN_SOC_IDLE_SLEEP"
...
#if defined(SL_CATALOG_POWER_MANAGER_PRESENT) && defined(CORTEXM3_EFR32)
#include "sl_power_manager.h"
#endif //SL_CATALOG_POWER_MANAGER_PRESENT && CORTEXM3_EFR32
...
void emberAfPluginIdleSleepTickCallback(void)
{
...
#if !defined(EMBER_AF_HAS_RX_ON_WHEN_IDLE_NETWORK)
  ...
  #ifdef EMBER_AF_PLUGIN_RTOS_COMMON
  // Check if the reason that we cannot idle / sleep is because we are not joined to a network
  // If so, yield the Zigbee task until the next scheduled event so that other tasks
  // can continue to run
  else if (emAfCheckStayAwakeWhenNotJoined() ) {
    // Check when the next scheduled zigbee stack event is supposed to occur
    // If there are none, pend for max allowed time
    // Note that this needs to work during joining as well as in a "not-joined" state
    uint32_t durationMs = (!emberCurrentStackTasks()
                           ? MAX_INT32U_VALUE
                           : emberMsToNextStackEvent());

    //check AF events as well and set duration accordingly
    durationMs = emberMsToNextEvent(emAfEvents, durationMs);

    // Since the application wants to stay awake, pass false to the
    // sleep handler so it simply pends the task instead of changing
    // energy modes
    if ( durationMs ) {
      emberAfPluginIdleSleepRtosCallback(&durationMs, false);
    }
  }
  #endif //#ifdef EMBER_AF_PLUGIN_RTOS_COMMON
...
#if defined(SL_CATALOG_POWER_MANAGER_PRESENT) && defined(CORTEXM3_EFR32) && defined(SL_KERNEL_PRESENT)
  // We don't want to go deeper than EM2 sleep mode in general.
  sl_power_manager_add_em_requirement(SL_POWER_MANAGER_EM2);
#endif //SL_CATALOG_POWER_MANAGER_PRESENT && CORTEXM3_EFR32 && SL_KERNEL_PRESENT
}
