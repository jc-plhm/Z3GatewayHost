...
static coap_context_t *get_context(const char *node, const char *port)
{
  ...
  for (rp = result; rp != NULL; rp = rp->ai_next) {
    ...
    if (rp->ai_addrlen <= sizeof(addr.addr)) {
      ...
      (void) memcpy(&addr.addr, rp->ai_addr, rp->ai_addrlen);
...
static int resolve_address(const str *server, struct sockaddr *dst)
{
  ...
  if (server->length) {
    (void) memcpy(addrstr, server->s, server->length);
  } else {
    (void) memcpy(addrstr, "localhost", 9);
  }

  (void) memset((char *)&hints, 0, sizeof(hints));
  ...
  for (ainfo = res; ainfo != NULL; ainfo = ainfo->ai_next) {
    switch (ainfo->ai_family) {
      ...
      case AF_INET:
        ...
        (void) memcpy(dst, ainfo->ai_addr, len);
        goto finish;
      default:
        ;
    }
...
coap_list_t *new_option_node(unsigned short key, unsigned int length, unsigned char *data)
{
  ...
  if (node) {
    ...
    (void) memcpy(COAP_OPTION_DATA(*option), data, length);
  } else {
