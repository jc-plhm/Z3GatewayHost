                                                   ...
                                                   uint8_t channel)
{
  ...
  if (status == EMBER_SUCCESS) {
    ...
    emberAfCorePrintln("%p: Form. Channel: %d. Status: 0x%X",
                       ...
                       channel,
                       status);
    emberAfPluginNetworkCreatorCompleteCallback((const EmberNetworkParameters *) &networkParameters,
                                                false);
  }
...
static EmberStatus scheduleScans()
{
  ...
  (void)emberSetRadioPower(EMBER_AF_PLUGIN_NETWORK_CREATOR_RADIO_POWER);
