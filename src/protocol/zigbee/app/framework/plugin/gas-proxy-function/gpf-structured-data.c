                           ...
                           GpfSampleLog *sampleLog)
{
  ...
  for (i = 0, entryIndex = getFirstIndex(sampleLog);
    ...
    (void) emberAfPutInt24uInResp(sampleLog->entries[entryIndex].sample);
    includedSamples++;
  }
                      ...
                      GpfTopUpLog *topUpLog)
{
  ...
  while (numberOfRecords == 0 || includedRecords < numberOfRecords) {
    ...
    (void) emberAfPutStringInResp(topUpLog->entries[indexToSend].code);
    (void) emberAfPutInt32uInResp(topUpLog->entries[indexToSend].amount);
    (void) emberAfPutInt32uInResp(topUpLog->entries[indexToSend].time);
                     ...
                     GpfDebtLog *debtLog)
{
  ...
  while (numberOfDebts == 0 || includedDebts < numberOfDebts) {
    ...
    (void) emberAfPutInt32uInResp(debtLog->entries[indexToSend].collectionTime);
    (void) emberAfPutInt32uInResp(debtLog->entries[indexToSend].amountCollected);
    (void) emberAfPutInt8uInResp(debtLog->entries[indexToSend].debtType);
    (void) emberAfPutInt32uInResp(debtLog->entries[indexToSend].outstandingDebt);
