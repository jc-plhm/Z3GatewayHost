...
// Custom command arguement description
...
static PGM_P PGM gpAppGpSendGpRawArguments[] = {
  "Channel",
...
EmberCommandEntry emberAfCustomCommands[] = {
  ...
  emberCommandEntryActionWithDetails("gpAppGpSendGpRaw", gpAppGpSendGpRaw, "uub", "Sends a raw packet using App rail handle", gpAppGpSendGpRawArguments),
                                  ...
                                  bool replyExpected)
{
  ...
  emberAfCorePrintln("replyExpected = 0x%X replyLength = 0x%X", replyExpected, replyLength);
...
static void gpAppGpTxQueueSet(void)
{
  ...
  commandPayload[idx++] = (uint8_t)emberUnsignedCommandArgument(2); // command Id
  const uint8_t *data = emberStringCommandArgument(3, &dataLength); //mpdu
  if (data
      && dataLength) {
...
static void gpAppGpSetTxQueueSize(void)
{
  ...
  idx += sizeof(uint8_t);
  uint16_t maxTxGpQueueSize = emberUnsignedCommandArgument(0);
