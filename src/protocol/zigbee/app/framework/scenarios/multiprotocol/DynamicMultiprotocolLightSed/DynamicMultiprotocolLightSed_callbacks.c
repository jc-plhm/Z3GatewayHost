...
void emberAfPluginBleEventCallback(sl_bt_msg_t *evt)
{
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_sm_confirm_passkey_id: {
      sl_bt_evt_sm_confirm_passkey_t * confirm_passkey_evt =
        (sl_bt_evt_sm_confirm_passkey_t*) &(evt->data);
      emberAfCorePrint("Confirm that the passkeys match. Connection handle=0x%x, passkey=%u",
                       confirm_passkey_evt->connection,
                       confirm_passkey_evt->passkey);
      emberAfCorePrintln("");
    }
    break;
    case sl_bt_evt_sm_passkey_display_id: {
      sl_bt_evt_sm_passkey_display_t * passkey_display_evt =
        (sl_bt_evt_sm_passkey_display_t*) &(evt->data);
      emberAfCorePrint("Enter passkey to other device. Connection handle=0x%x, passkey=%u",
                       passkey_display_evt->connection,
                       passkey_display_evt->passkey);
      emberAfCorePrintln("");
    }
    break;
    case sl_bt_evt_system_error_id: {
      sl_bt_evt_system_error_t * system_error_evt =
        (sl_bt_evt_system_error_t*) &(evt->data);
      emberAfCorePrint("System error. Error=0x%2x, data: ",
                       system_error_evt->reason);
      if (system_error_evt->data.len != 0) {
        emberAfCorePrint("0x%02x", system_error_evt->data.data[0]);
        for (uint8_t i = 1; i < system_error_evt->data.len; i++) {
          emberAfCorePrint(", 0x%02x", system_error_evt->data.data[i]);
        }
      }
      emberAfCorePrintln("");
    }
...
void emberAfHalButtonIsrCallback(int8u button, int8u state)
{
  ...
  // Upon getting a button interrupt, make sure we set zigbee task to active
