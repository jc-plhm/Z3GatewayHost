...
// documented in event_control/event.h (silabs_core)
...
{
  if (index != 0xFF) {
    (void) memset(&localDeviceTable[index], 0xFF, sizeof(localDeviceTable_t));
      ...
      if (localDeviceTableLookUpByEui(bindingEntry.identifier) == LOCAL_DEVICE_TABLE_ENTRY_NOT_FOUND) {
        if ((bindingEntry.type == EMBER_UNICAST_BINDING)
          ...
          if (freeEntry != LOCAL_DEVICE_TABLE_FULL) {
            (void) memcpy(&localDeviceTable[freeEntry], bindingEntry.identifier, EUI64_SIZE);
