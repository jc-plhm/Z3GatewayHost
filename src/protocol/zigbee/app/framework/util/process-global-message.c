...
{
  uint16_t i;
  emberAfServiceDiscoveryPrint("Discover Commands response (complete: %c), %p IDs: ",
                               (discoveryComplete
                                ? 'y'
                                : 'n'),
                               (generated
...
bool emAfProcessGlobalCommand(EmberAfClusterCommand *cmd)
{
  ...
  (void) emberAfPutInt8uInResp(frameControl);

  if (cmd->mfgSpecific) {
    mfgCodePtr = (uint8_t *)emberAfPutInt16uInResp(cmd->mfgCode);
  }
  ...
  switch (zclCmd) {
    ...
    case ZCL_READ_ATTRIBUTES_COMMAND_ID:
    {
      ...
      (void) emberAfPutInt8uInResp(ZCL_READ_ATTRIBUTES_RESPONSE_COMMAND_ID);
    ...
    case ZCL_WRITE_ATTRIBUTES_UNDIVIDED_COMMAND_ID:
    {
      ...
      (void) emberAfPutInt8uInResp(ZCL_WRITE_ATTRIBUTES_RESPONSE_COMMAND_ID);

      // Go through the message until there are no more attrID/type/data
      while (msgIndex < msgLen - 3u) {
        attrId = emberAfGetInt16u(message, msgIndex, msgLen);
        dataType = emberAfGetInt8u(message, msgIndex + 2u, msgLen);
    ...
    case ZCL_DISCOVER_ATTRIBUTES_EXTENDED_COMMAND_ID:
    {
      ...
      numberAttributes = emberAfGetInt8u(message, msgIndex + 2u, msgLen);
      ...
      (void) emberAfPutInt8uInResp(false);
    ...
    case ZCL_READ_ATTRIBUTES_RESPONSE_COMMAND_ID:
      ...
      if (clusterId == ZCL_TIME_CLUSTER_ID) {
        if (emAfSyncingTime
            ...
            && (emberAfGetInt8u(message, msgIndex + 2u, msgLen)
                == EMBER_ZCL_STATUS_SUCCESS)
            && (emberAfGetInt8u(message, msgIndex + 3u, msgLen)
      ...
      if (clusterId == ZCL_KEY_ESTABLISHMENT_CLUSTER_ID
          ...
          && (emberAfGetInt8u(message, msgIndex + 2u, msgLen)
              == EMBER_ZCL_STATUS_SUCCESS)
          && ((emberAfGetInt8u(message, msgIndex + 3u, msgLen) == ZCL_ENUM16_ATTRIBUTE_TYPE)
              || (emberAfGetInt8u(message, msgIndex + 3u, msgLen) == ZCL_BITMAP16_ATTRIBUTE_TYPE))) {
        uint16_t suite = emberAfGetInt16u(message, msgIndex + 4u, msgLen);
        emberAfPluginKeyEstablishmentReadAttributesCallback(suite);
      }
#endif

#if defined(EMBER_AF_PLUGIN_TEST_HARNESS)
      emberAfPluginTestHarnessReadAttributesResponseCallback(clusterId,
                                                             message + msgIndex,
                                                             msgLen - msgIndex);
#endif
      ...
      emberAfPluginSimpleMeteringClusterReadAttributesResponseCallback(clusterId,
                                                                       message + msgIndex,
                                                                       msgLen - msgIndex);
#endif
  ...
  #if defined(EMBER_AF_PLUGIN_TEST_HARNESS)
      emberAfPluginTestHarnessWriteAttributesResponseCallback(clusterId,
                                                              message + msgIndex,
                                                              msgLen - msgIndex);
  #endif
    ...
    case ZCL_DEFAULT_RESPONSE_COMMAND_ID:
    {
      ...
      emberAfClusterDefaultResponseWithMfgCodeCallback(cmd->apsFrame->destinationEndpoint,
                                                       clusterId,
                                                       commandId,
                                                       status,
                                                       clientServerMask,
    ...
    {
      bool discoveryComplete = (emberAfGetInt8u(message, msgIndex, msgLen) != 0u);
      msgIndex++;
    ...
    case ZCL_DISCOVER_COMMANDS_GENERATED_COMMAND_ID:
    {
      ...
      if ( zclCmd == ZCL_DISCOVER_COMMANDS_RECEIVED_COMMAND_ID ) {
        (void) emberAfPutInt8uInResp(ZCL_DISCOVER_COMMANDS_RECEIVED_RESPONSE_COMMAND_ID);
        flag = false;
      } else {
        (void) emberAfPutInt8uInResp(ZCL_DISCOVER_COMMANDS_GENERATED_RESPONSE_COMMAND_ID);
        flag = true;
      }
  ...
  kickout:
  emberAfSendDefaultResponse(cmd, EMBER_ZCL_STATUS_UNSUP_COMMAND);
  return true;
}
