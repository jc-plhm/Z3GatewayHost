...
void printCli(void)
{
  ...
  (void) emberSerialPrintf(cliSerialPort, "-------------------------\r\n");
  (void) emberSerialPrintf(cliSerialPort, "currentSerialBuff = %x\r\n",
                           currentSerialBuff);
  (void) emberSerialWaitSend(cliSerialPort);
  ...
  for (i = 0; i < CLI_MAX_NUM_SERIAL_ARGS; i++) {
    (void) emberSerialPrintf(cliSerialPort, "%x:%x: ", i, serialBuffPosition[i]);

    for (j = 0; j < CLI_MAX_SERIAL_CMD_LINE; j++) {
      (void) emberSerialPrintf(cliSerialPort, "%x ", serialBuff[i][j]);
      (void) emberSerialWaitSend(1);
    }
    (void) emberSerialPrintf(1, "\r\n");
    (void) emberSerialWaitSend(1);
  }
}
...
void cliProcessSerialInput(void)
{
  ...
  if (emberSerialReadByte(cliSerialPort, &ch) == 0) {
    switch (ch) {
      ...
      case ' ':
        // echo
        (void) emberSerialPrintf(cliSerialPort, "%c", ch);
      ...
      default:
        ...
        if (pos < CLI_MAX_SERIAL_CMD_LINE) {
          ...
          //(void) emberSerialPrintf(cliSerialPort, "add %x to %x at %x\r\n", ch,
...
void processSerialLine(void)
{
  ...
  if (serialBuffPosition[0] > 0) {
    ...
    if (match == false) {
      (void) emberSerialPrintf(cliSerialPort, "command not recognized\r\n");
    }
  }
...
void clearSerialLine(void)
{
  ...
  (void) emberSerialPrintf(cliSerialPort, "%p>", cliPrompt);
#endif
}
