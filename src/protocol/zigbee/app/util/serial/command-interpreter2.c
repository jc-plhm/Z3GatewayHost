...
static void echoPrint(void)
{
  ...
  for (; tokenNum < commandState.tokenCount; tokenNum++ ) {
    ...
    (void) emberSerialWriteData(APP_SERIAL, ptr, len);
    (void) emberSerialPrintf(APP_SERIAL, " ");
  }
  (void) emberSerialPrintf(APP_SERIAL, "\r\n");
}
...
static void callCommandAction(void)
{
  ...
  if (waitingForPassword == CMD_SET_NEW_PASSWORD) {
    ...
    if (tmpPassword && tmpPassword < 0xFFFFFFFF) {
      ...
      (void) emberSerialPrintf(APP_SERIAL, "\r Invalid password value \n");
    }
    emberEventControlSetDelayQS(emberAfPluginCliPasswordProtectionTimingEventControl, passwordTick);
    ...
    if (tmpPassword != (uint32_t)emberUnsignedCommandArgument(0)) {
      ...
      (void) emberSerialPrintf(APP_SERIAL, "\r password is set, proceed to enter any CLI command\n");
    }
    emberEventControlSetDelayQS(emberAfPluginCliPasswordProtectionTimingEventControl, passwordTick);

    return;
  } else if (waitingForPassword == CMD_PASSWORD_LOCKOUT) {
    (void) emberSerialPrintf(APP_SERIAL, "\r CLI is locked untill the wait time of %d seconds is expired.\n", passwordTick / 4);
    return;
  } else if (waitingForPassword == CMD_WAITING_FOR_PASSWORD) {
    if (!isPasswordCorrect((uint32_t) emberUnsignedCommandArgument(0))) {
      ...
      if (wrongPasswordsEntered >= wrongPasswordThreshold) {
        ...
        waitingForPassword = CMD_PASSWORD_LOCKOUT;
        setPasswordTick();
        (void) emberSerialPrintf(APP_SERIAL, "\r Password lockout: Need to wait for %d min to try again.\n",
                                 passwordTick / (4 * 60));
#endif
      } else {
        (void) emberSerialPrintf(APP_SERIAL, "\r Need to wait for %d seconds to try %d remaining password attempts.\n",
      ...
      setPasswordTick();
      waitingForPassword = CMD_PASSWORD_IS_SET;
      (void) emberSerialPrintf(APP_SERIAL, "\r Correct password, proceed to enter any CLI command\n");
      goto kickout2;
    }
  }
   #endif
...
bool emberStringToHostOrderIpv4Address(const uint8_t* string, uint32_t* hostOrderIpv4Address)
{
  ...
  *hostOrderIpv4Address = 0;
  while (*string != '\0' && *string != '"') {
    if (*string == '.') {
      if (!valueFound) {
        (void) emberSerialPrintfLine(APP_SERIAL, "Error: No value found before '.'");
        return false;
      }
      *hostOrderIpv4Address += (uint8_t)quad;
      *hostOrderIpv4Address <<= 8;
//      (void) emberSerialPrintfLine(APP_SERIAL, "Debug: Finished quad %d. IP: 0x%4X", quad, *hostOrderIpv4Address);
      quad = 0;
      quadCount++;
      valueFound = false;
    } else if (*string < '0' || *string > '9') {
      (void) emberSerialPrintfLine(APP_SERIAL, "Error: Invalid char '%c' in IP address", *string);
      return false;
    } else {
      valueFound = true;
      quad *= 10;
      quad += (*string - ASCII_0);
//      (void) emberSerialPrintfLine(APP_SERIAL, "Debug: Quad is %d", quad);
      if (quad > 255) {
        (void) emberSerialPrintfLine(APP_SERIAL, "Error: Invalid value of %d in IP address", quad);
        return false;
      }
    }
    string++;
  ...
  if (quadCount != 4) {
    (void) emberSerialPrintfLine(APP_SERIAL, "Error: Expected 4 quads, got %d.", quadCount);
    return false;
  }
//  (void) emberSerialPrintfLine(APP_SERIAL, "Debug: Finished IP parsing.");
  *hostOrderIpv4Address += (uint8_t)quad;
  return true;
}
                              ...
                              EmberCommandEntry *entry)
{
  ...
  (void) emberSerialPrintf(APP_SERIAL, "%p", entry->name);
...
{
  (void) emberSerialPrintf(APP_SERIAL, "%p\r\n", emberCommandErrorNames[status]);
  ...
  if (emberCurrentCommand == NULL) {
    ...
    for (tokenNum = 0; tokenNum < commandState.argOffset - 1; tokenNum++) {
      ...
      for (i = 0; i < tokenLength(tokenNum); i++) {
        (void) emberSerialPrintf(APP_SERIAL, "%c", finger[i]);
      }
      (void) emberSerialPrintf(APP_SERIAL, " ");
    }
