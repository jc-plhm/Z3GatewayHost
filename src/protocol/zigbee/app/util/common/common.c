...
void helpCommand(void)
{
  ...
  while (emberCommandTable[commandIndex].action != NULL) {
    (void) emberSerialPrintf(serialPort,
                             "%p %p",
    ...
    if (emberCommandTable[commandIndex].description != NULL) {
      (void) emberSerialPrintf(serialPort, " - %p",
...
{
  (void) emberSerialPrintf(serialPort, "%p ", applicationString);
  printLittleEndianEui64(serialPort, emberGetEui64());
  (void) emberSerialWaitSend(serialPort);
  (void) emberSerialPrintf(serialPort, " (%2x)", emberGetNodeId());
  if ( emberStackIsUp() ) {
    ...
    (void) emberSerialPrintf(serialPort, " Ember channel %d",
                             networkParams.radioChannel);
    (void) emberSerialPrintf(serialPort, " power %d PAN ID %2x XPID ",
    ...
    (void) emberSerialPrintfLine(serialPort, "Additional Interfaces:");
    if ((phyInterfaceCount > 1) && (255 != phyInterfaceCount)) {
      ...
      for (i = 1; i < phyInterfaceCount; ++i) {
        ...
        if (status == EMBER_SUCCESS ) {
          (void) emberSerialPrintfLine(serialPort, " %d. Page %d Channel %d Power %d ", i,
                                       multiPhyRadioParams.radioPage,
                                       multiPhyRadioParams.radioChannel,
                                       multiPhyRadioParams.radioTxPower);
        } else {
          (void) emberSerialPrintfLine(serialPort, " %d. Offline, status 0x%x", i, status);
        }
      }
    } else {
      (void) emberSerialPrintfLine(serialPort, " None");
    }
#endif
  } else {
    (void) emberSerialPrintfLine(serialPort, " offline");
  }
}
...
void getRadioParameters(void)
{
  ...
  if (status == EMBER_SUCCESS) {
    (void) emberSerialPrintfLine(serialPort, "Power %d Page %d Channel %d", params.radioTxPower,
                                 params.radioPage, params.radioChannel);
  } else {
    (void) emberSerialPrintfLine(serialPort, "failed to get radio parameters error 0x%x", status);
  }
}
...
{
  (void) emberSerialPrintf(port, "(<)%X%X%X%X%X%X%X%X",
...
void printHexByteArray(uint8_t port, uint8_t *byteArray, uint8_t length)
{
  ...
  while ((index + 4) <= length) {
    (void) emberSerialPrintf(port, " %X %X %X %X",
  ...
  while (index < length) {
    (void) emberSerialPrintf(port, " %X", byteArray[index]);
    index += 1;
  }
}
                        ...
                        const char * bad)
{
  if (status == EMBER_SUCCESS) {
    if ( good != NULL ) {
      (void) emberSerialPrintfLine(serialPort, "%p", good);
    }
  } else {
    (void) emberSerialPrintfLine(serialPort, "%p, status:0x%x", bad, status);
  }
}
...
{
  (void) emberSerialPrintf(serialPort, "%p", operation);
...
{
  UNUSED uint8_t nwkIndex = emberUnsignedCommandArgument(0);
  (void) emberSetCurrentNetwork(nwkIndex);
}

void getCurrentNwkIndexCommand(void)
{
  (void) emberSerialPrintfLine(serialPort,
                               "Current network index %d",
                               emberGetCurrentNetwork());
}
...
{
  (void) emberSerialPrintfLine(serialPort, "Error: %p", message);
}

void printErrorMessage2(const char * message, const char * message2)
{
  (void) emberSerialPrintfLine(serialPort, "Error: %p %p", message, message2);
}
