                                       ...
                                       EmberMessageBuffer payload)
{
  ...
  finger++; // skip over sequence number, which is overridden by the mac anyway
                             ...
                             InterPanHeader *headerData)
{
  ...
  if ((macFrameControl & 0xFFDF)
    ...
    //(void) emberSerialPrintf(1, "bad MAC frame control %2x\r\n", macFrameControl);
    return 0;
  }
  ...
  if (HIGH_LOW_TO_INT(finger[1], finger[0]) != STUB_NWK_FRAME_CONTROL) {
    ...
    //(void) emberSerialPrintf(1, "bad NWK frame control %x%x\r\n", *(finger - 1), *finger);
    return 0;
  }
  finger += 2;
  ...
  if (headerData->messageType == INTER_PAN_MULTICAST) {
    ...
    //(void) emberSerialPrintf(1, "bad APS frame control %2x\r\n", headerData->messageType);
    return 0;
  }
...
//          haveBytes);
  ...
  if (finger - header <= haveBytes) {
    //(void) emberSerialPrintf(1, "ret %x\r\n", startOffset + finger - header);
    return startOffset + finger - header;
  } else {
    //(void) emberSerialPrintf(1, "ret zero\r\n");
    return 0;
  }
}
