/***************************************************************************//**
 * @file
 * @brief A simple Host application for testing the EM260. A serial
...
{
  (void) emberSerialPrintf(SERIAL_PORT, "usage: " EXE_NAME
                           " [--logfile <name>]"
                           " [--semaphore[-create] <name>]\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  example: " EXE_NAME
                           " --logfile spi.log"
...
static void printCommandStatus(const char * commandName,
                               EmberStatus status);
...
uint16_t *heapMemory = NULL;
uint16_t hostHeapMemory[4000];
uint32_t heapMemorySize = 4000;
...
static void getConfigCommand(EmberCommandState *state)
{
  ...
  (void) emberSerialPrintf(SERIAL_PORT,
...
{
  ezspNop();
  (void) emberSerialPrintf(SERIAL_PORT, "nop success.\r\n");
}
...
static void echoCommand(EmberCommandState *state)
{
  ...
  if (echoLength == length
    ...
    (void) emberSerialPrintf(SERIAL_PORT, "%d byte echo success.\r\n", length);
  } else {
    (void) emberSerialPrintf(SERIAL_PORT, "%d byte echo failed.\r\n", length);
  }
}
...
static void payloadCommand(EmberCommandState *state)
{
  ...
  (void) emberSerialPrintf(SERIAL_PORT, "Payload type set to %u.\r\n", payloadType);
}
...
static void testCommand(EmberCommandState *state)
{
  if (spiTestTotalCount == 0) {
    ...
    (void) emberSerialPrintf(SERIAL_PORT, "Running test %d of %d.\r\n",
...
static void sleepCommand(EmberCommandState *state)
{
  ...
  if (newSleepMode > 4) {
    (void) emberSerialPrintf(SERIAL_PORT, "Invalid sleep mode (must be 0 - 4)\r\n");
    return;
  }

  if (newSleepMode == 4) {
    (void) emberSerialPrintf(SERIAL_PORT, "Sleeping the host...");
    (void) emberSerialWaitSend(SERIAL_PORT);
    halPowerDown();
    halSleep(SLEEPMODE_POWERDOWN);
    halPowerUp();
    (void) emberSerialPrintf(SERIAL_PORT, " Awake!\r\n");
    (void) emberSerialWaitSend(SERIAL_PORT);
  } else {
    (void) emberSerialPrintf(SERIAL_PORT, "Changed sleep mode from %p to %p.\r\n",
...
static void randomCommand(EmberCommandState *state)
{
  ...
  (void) emberSerialPrintf(SERIAL_PORT, "%u\r\n", random);
}
...
static void setKeyCommand(EmberCommandState *state)
{
  ...
  if (MEMCOMPARE(emberKeyContents(&(securityKey)),
    ...
    (void) emberSerialPrintf(SERIAL_PORT, "Error reading back network key.\r\n");
  }
}
...
static void countersCommand(EmberCommandState *state)
{
  ...
  (void) emberSerialPrintf(SERIAL_PORT, "Counters: ");
  for (i = 0; i < EMBER_COUNTER_TYPE_COUNT; i++) {
    (void) emberSerialPrintf(SERIAL_PORT, "%2X ", values[i]);
  }
  (void) emberSerialPrintf(SERIAL_PORT, ".\r\n");
}
...
static void errorCommand(EmberCommandState *state)
{
  ...
  (void) emberSerialPrintf(SERIAL_PORT, "Host will%p assert on EZSP error.\r\n",
...
static void setNetworkCommand(EmberCommandState *state)
{
  ...
  if (status == EMBER_SUCCESS) {
    ...
    (void) emberSerialPrintf(SERIAL_PORT, "Host set network index failed\r\n");
  }
}
...
static void getXNcpInfoCommand(EmberCommandState *state)
{
  ...
  if (status == EMBER_SUCCESS) {
    ...
    (void) emberSerialPrintf(SERIAL_PORT, "XNCP library not present\r\n");
  }
}
...
//  //if (status != EMBER_SUCCESS)
//  //{
//  //  (void) emberSerialPrintf(SERIAL_PORT, "get_eui64: Get EUI64 failed with error 0x%X\r\n", status);
//  //}
//  //else
//  //{
//    (void) emberSerialPrintf(SERIAL_PORT, "get_eui64: EUI64 ");
...
{
  (void) emberSerialPrintf(SERIAL_PORT, "net_state: Network state 0x%X\r\n", ezspNetworkState());
}
...
static void networkParamsCommand(EmberCommandState *state)
{
  ...
  if (status != EMBER_SUCCESS) {
    (void) emberSerialPrintf(SERIAL_PORT, "net_param: Get network params failed with error 0x%X\r\n", status);
  } else {
    int i;

    (void) emberSerialPrintf(SERIAL_PORT, "net_param: Node type 0x%X\r\n", nodeType);
    (void) emberSerialPrintf(SERIAL_PORT, "net_param: Extended PAN ID 0x");
    for (i = 0; i < 8; i++) {
      (void) emberSerialPrintf(SERIAL_PORT, "%X", params.extendedPanId[i]);
    }
    (void) emberSerialPrintf(SERIAL_PORT, "\r\n");
    (void) emberSerialPrintf(SERIAL_PORT, "net_param: PAN ID 0x%2X\r\n", params.panId);
    (void) emberSerialPrintf(SERIAL_PORT, "net_param: Radio TX power %d\r\n", params.radioTxPower);
    (void) emberSerialPrintf(SERIAL_PORT, "net_param: Radio channel %d\r\n", params.radioChannel);
    (void) emberSerialPrintf(SERIAL_PORT, "net_param: Join method 0x%X\r\n", params.joinMethod);
    (void) emberSerialPrintf(SERIAL_PORT, "net_param: Network manager ID 0x%2X\r\n", params.nwkManagerId);
    (void) emberSerialPrintf(SERIAL_PORT, "net_param: Network update ID 0x%X\r\n", params.nwkUpdateId);
    (void) emberSerialPrintf(SERIAL_PORT, "net_param: Channel mask 0x%4X\r\n", params.channels);
  }
}
...
static void networkLeaveCommand(EmberCommandState *state)
{
  ...
  if (status == EMBER_SUCCESS) {
    (void) emberSerialPrintf(SERIAL_PORT, "net_leave: Left network\r\n");
  } else {
    (void) emberSerialPrintf(SERIAL_PORT, "net_leave: Error 0x%X\r\n", status);
  }
}
...
{
  cleanupBeforeExit();
  (void) emberSerialPrintf(SERIAL_PORT, "Exiting\r\n");
  exit(0);
}
#else
...
static void helpCommand(EmberCommandState *state)
{
  ...
  (void) emberSerialPrintf(SERIAL_PORT, "\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "Available commands (name [arguments] - description):\r\n");
  ...
  (void) emberSerialPrintf(SERIAL_PORT, "\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "Arguments types:\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  b:   buffer\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  u1:  one-byte unsigned\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  u2:  two-byte unsigned\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  u4:  four-byte unsigned\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  s1:  one-byte signed\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  s2:  two-byte signed\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  *:   zero or more of the previous type.\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "       If used, this must be the last specifier.\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  Buffer arguments are given as a string \"...\". There is currently no\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  way to include quote characters \' or \" in strings arguments. Buffer\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  arguments can also be given as a hex digit string using curly braces as\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  in {1A2B3C}. The hex digits are converted to bytes in pairs.\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  Integer arguments can be either decimal or hexidecimal. Decimal is the\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  default and a 0x prefix indicates a hexidecimal integer.\r\n");

  (void) emberSerialPrintf(SERIAL_PORT, "\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "Examples:\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  help - Print this help dialog\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  nop - Send an EZSP NOP frame to the NCP\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  echo 81 - Send an EZSP ECHO frame with 81 bytes of data to the NCP\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  echo 0x81 - Send an EZSP ECHO frame with 129 bytes (maximum value) of data to the NCP\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  test 10 0x10 100 - Test EZSP-SPI to the NCP by sending 10 frames of 16 bytes each 100 ms apart\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  get_addr - Get NCP's ZigBee short ID\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  set_addr 0xFFFD - Set NCP's ZigBee short ID to 0xFFFD\r\n");
  (void) emberSerialPrintf(SERIAL_PORT, "  exit - Exit the program\r\n");
...
{
  (void) emberSerialPrintf(SERIAL_PORT, "Error: %p\r\n",
                           emberCommandErrorNames[state->error]);
  (void) emberSerialPrintf(SERIAL_PORT, "Type help for a list of commands\r\n");
}
...
extern uint16_t *emHeapLimit;

int main(MAIN_FUNCTION_PARAMETERS)
{
  heapMemory = hostHeapMemory; // local heap
  ...
  emHeapLimit = heapMemory + heapMemorySize;
  emInitializeBuffers();
  INTERRUPTS_ON();
  ...
  (void) emberSerialPrintf(SERIAL_PORT, APP_NAME " Application\r\n");
  ...
  for (tries = 1; initResult != EZSP_SUCCESS && tries < EZSP_INIT_TRIES; tries++) {
    (void) emberSerialPrintf(SERIAL_PORT, "ezspInit failed, retrying\r\n");
    initResult = ezspInit();
  }
  ...
  (void) emberSerialPrintf(SERIAL_PORT, "NCP Initialized\r\n");
  run();
...
static void run(void)
{
  while (true) {
    ...
    if (em260WokeUp) {
      (void) emberSerialPrintf(SERIAL_PORT, "EM260 is awake.\r\n");
      em260WokeUp = false;
    }
    ezspFragmentTick();
  }
...
{
  if (status == EMBER_SUCCESS) {
    (void) emberSerialPrintf(SERIAL_PORT, "%p success.\r\n", commandName);
  } else {
    (void) emberSerialPrintf(SERIAL_PORT, "Error: %p status 0x%X.\r\n",
                             commandName, status);
  }
}
                                ...
                                uint8_t *messageContents)
{
  ...
  if (apsFrame->profileId == EMBER_ZDO_PROFILE_ID) {
    if (apsFrame->clusterId == NETWORK_ADDRESS_RESPONSE
      ...
      for (i = 0; i < EUI64_SIZE; i++) {
        (void) emberSerialPrintf(SERIAL_PORT, "%X", eui64[i]);
      }
      (void) emberSerialPrintf(SERIAL_PORT, ".\r\n");
      ...
      inputCount = *finger;
      finger++;
      (void) emberSerialPrintf(SERIAL_PORT, "%u in: ", inputCount);
      for (i = 0; i < inputCount; i++) {
        (void) emberSerialPrintf(SERIAL_PORT, "%2X ", HIGH_LOW_TO_INT(finger[1], finger[0]));
        finger += 2;
      }
      outputCount = *finger;
      finger++;
      (void) emberSerialPrintf(SERIAL_PORT, ", %u out: ", outputCount);
  ...
  if (apsFrame->options & EMBER_APS_OPTION_FRAGMENT) {
    ...
    (void) emberSerialPrintf(SERIAL_PORT, "fragmented: length %d, contents %p.\r\n",
